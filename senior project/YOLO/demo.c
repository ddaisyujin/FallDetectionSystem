...
void wedConnect()
{
	...
	clientService.sin_addr.s_addr = inet_addr("127.0.0.1");//웹 서버주소 ( 연결할 ip )
...
static volatile int run_drawing = 0;

void *sock_in_thread(void *ptr) {
	...
	int benchmark, int benchmark_layers)
{
	...
	custom_thread_t sock_thread = NULL;
	...
	if (custom_create_thread(&sock_thread, 0, sock_in_thread, 0)) error("Thread creation failed");
	...
	while (1) {
		++count;
		{
			...
			if (!benchmark && !dontdraw_bbox) draw_detections_cv_v3(show_img, local_dets, local_nboxes, demo_thresh, demo_names, demo_alphabet, demo_classes, demo_ext_output, &classid);
